...
| `TIMELINE-V1.28.html` | **Active HTML frontend** — open this in browser. Persists active tab and task-table sort column/direction across loads. |
| `TIMELINE-V1.27.html` | Previous HTML version. Keep for reference. |
...
## Version History
- **V1.28** (2026-07-07) — HTML only. **Persist active tab and sort order**: `collectSettings()` now saves `sortColumn`, `sortDirection`, and `currentTab`. `applySettings()` restores all three — sort indicators update, tab buttons and panel visibility switch to match. Load callback preserves `sortState` when settings have a `sortColumn` (instead of always resetting to null), then calls `applySort()` + `updateSortIndicators()` on the freshly-loaded task list. Backend unchanged (no redeploy needed).
- **V1.27** (2026-07-06) — HTML only. **Fix project title cutoff with Date/Duration columns**: `renderCornerSvg()` now bases title and subtitle word-wrap on `leftLabelWidth` instead of `sideWidth`. Previously, when Date and Duration columns were both enabled, `sideWidth` was 220 px wider, so the word-wrap allowed a wider line, the title rendered as one long line extending into the date-column area, and the date/duration column background rects painted over it, making the title appear cut off. Backend unchanged (no redeploy needed).
