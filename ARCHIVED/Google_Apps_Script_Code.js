...
//
//  Reads directly from your existing "PROJECT TASK LIST" tab.
//  Only tasks that have BOTH a Start Date and End Date appear
//  on the Gantt. Saving pushes date + status changes back to
//  those same rows — everything else is left untouched.
//
//  SETUP (first time):
...
//
//  UPDATING (if you already deployed v1):
//  Deploy → Manage deployments → pencil icon → New version → Deploy
//  (the URL stays the same)
// ============================================================

var SOURCE_SHEET = 'PROJECT TASK LIST';   // your existing tab name
...
function doGet(e) {
  try {
    return buildResponse(importFromTaskList());
  } catch (err) {
...
function doPost(e) {
  try {
    ...
    var msg = saveBackToTaskList(payload);
    return buildResponse({ success: true, message: msg });
  } catch (err) {
    return buildResponse({ success: false, error: err.toString() });
  }
}
