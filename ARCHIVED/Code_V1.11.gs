...
function importFromTaskList() {
  ...
  for (var row = hRow + 1; row < raw.length; row++) {
    ...
    // Milestones are single-point events (start == end is valid).
    // Only skip rows where end is strictly before start.
    if (endDate.getTime() < startDate.getTime()) continue;
    // Bars must have a non-zero duration.
    if (!isMilestone && endDate.getTime() === startDate.getTime()) continue;
...
var SETTINGS_KEYS = [
  ...
  'groupTint', 'groupHeaderColor', 'groupHeaderTint', 'groupHeaderTextColor',
...
function writeSettings(settings) {
  ...
  if (settings.appScriptURL) rows.push(['appScriptURL', settings.appScriptURL]);
