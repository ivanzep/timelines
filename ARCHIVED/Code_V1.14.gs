...
//
//  V1.13  2026-05-21
//    • onOpen() adds a permanent "📊 Gantt Timeline" custom menu to
//      the spreadsheet UI — no deployment needed for the menu itself.
//    • showWebAppUrl() — reads the deployed web app URL via
//      ScriptApp.getService().getUrl(), falls back to the sheetsUrl
//      row in GANTT SETTINGS tab. Shows a copyable HTML dialog.
//      If the script is not yet deployed, shows step-by-step
//      deployment instructions instead.
//    • showSetupHelp() — compact HTML dialog with the 5-step
//      setup procedure.
//
//  V1.05  2026-05-18
...
// ============================================================
//  SPREADSHEET MENU  (appears automatically when sheet opens)
// ============================================================

/**
 * Adds the "📊 Gantt Timeline" custom menu every time the sheet is opened.
 * This is a simple trigger — no deployment needed for the menu itself.
 */
function onOpen() {
...
/**
 * Shows the deployed web app URL in a copyable HTML popup.
 * Primary source: ScriptApp.getService().getUrl() (only works once deployed).
 * Fallback: the sheetsUrl row saved in GANTT SETTINGS tab by the HTML tool.
 * If neither is available, shows step-by-step deployment instructions.
 */
function showWebAppUrl() {
  ...
  // Fallback: read the URL saved by the HTML Gantt tool on its last Save
  if (!url) {
    try {
  ...
  if (url) {
    var html = HtmlService.createHtmlOutput(
      '<style>' +
        'body{font-family:Arial,sans-serif;padding:16px;font-size:13px;color:#1e293b}' +
        'h3{margin:0 0 10px;font-size:15px}' +
        'p{margin:4px 0 10px;color:#475569;font-size:12px}' +
        '#url{width:100%;padding:8px;border:1px solid #cbd5e1;border-radius:6px;' +
             'font-size:11px;background:#f8fafc;word-break:break-all;' +
             'box-sizing:border-box;resize:none;height:52px}' +
        'button{margin-top:10px;padding:7px 18px;background:#0ea5e9;color:#fff;' +
               'border:none;border-radius:6px;cursor:pointer;font-size:13px;' +
               'font-weight:600;width:100%}' +
        'button:hover{background:#0284c7}' +
        '#msg{font-size:11px;color:#16a34a;margin-top:6px;text-align:center;min-height:16px}' +
      '</style>' +
      '<h3>🔗 Web App URL</h3>' +
      '<p>Copy this URL and paste it into the Gantt tool\'s ⚙ Setup panel.</p>' +
    ...
    var msg =
      ...
      'After deploying, reopen this dialog to retrieve the URL automatically.';
    ui.alert('Web App Not Deployed Yet', msg, ui.ButtonSet.OK);
  }
}

/**
 * Shows a brief setup guide dialog.
 */
function showSetupHelp() {
  var html = HtmlService.createHtmlOutput(
    '<style>' +
      'body{font-family:Arial,sans-serif;padding:16px;font-size:12px;color:#1e293b;line-height:1.5}' +
      'h3{margin:0 0 8px;font-size:14px}' +
      'ol{margin:6px 0;padding-left:18px}' +
      'li{margin-bottom:4px}' +
      'code{background:#f1f5f9;padding:1px 4px;border-radius:3px;font-size:11px}' +
      '.note{color:#64748b;font-size:11px;margin-top:10px;border-top:1px solid #e2e8f0;padding-top:8px}' +
    '</style>' +
    '<h3>📊 Gantt Timeline — Setup</h3>' +
    '<ol>' +
      '<li>Open <b>Extensions → Apps Script</b></li>' +
      '<li>Paste the <code>Code_V1.13.gs</code> contents and save</li>' +
      '<li><b>Deploy → New deployment</b><br>' +
          'Type: <i>Web app</i> · Execute as: <i>Me</i> · Access: <i>Anyone</i></li>' +
      '<li>Click <b>📊 Gantt Timeline → Get Web App URL</b> and copy the URL</li>' +
      '<li>Open the Gantt HTML file, click ⚙, paste the URL, click Connect</li>' +
    '</ol>' +
    '<p class="note">After code changes: Deploy → Manage deployments → ✏ → New version → Deploy. The URL stays the same.</p>'
  ).setWidth(380).setHeight(240);
  SpreadsheetApp.getUi().showModalDialog(html, 'Gantt Timeline Setup');
}
  ...
  try {
    ...
    if (Object.keys(taskParams).length > 0) {
      result.tasks.forEach(function(t) {
        ...
        if (p.deps)   t.dependencies = p.deps;  // tilde-separated "GROUP|NAME" keys
      });
    }
...
//    DEPS   — tilde-separated list of "DISCIPLINE|TASKNAME" predecessor keys
//
