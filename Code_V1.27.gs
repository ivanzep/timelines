...
// ---- Tab names ----
// SOURCE_SHEET can be overridden at runtime by the taskSheetName setting saved in GANTT SETTINGS.
var SOURCE_SHEET      = 'PROJECT TASK LIST';             // master task data — never reformatted; overridable via taskSheetName setting
...
function doGet(e) {
  try {
    // Read settings first so taskSheetName can override SOURCE_SHEET before import.
    var savedSettings = readSettings();
...
function doPost(e) {
  ...
  try {
    ...
    // saveBackToTaskList uses SOURCE_SHEET to find the tab.
    if (payload.settings && payload.settings.taskSheetName) {
      SOURCE_SHEET = payload.settings.taskSheetName;
    }
